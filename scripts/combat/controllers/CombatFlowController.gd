...
func setup(context_ref: Node, ui_ref: CombatUIController, reward_flow_ref: RewardFlowController) -> void:
	context = context_ref
	ui = ui_ref
...
func log_turn_start() -> void:
	ui.append_battle_log("回合%d开始：你HP %d/%d，敌人HP %d/%d" % [
...
func log_turn_end() -> void:
	ui.append_battle_log("回合%d结算：你HP %d/%d 护甲%d（%s），敌人HP %d/%d 护甲%d" % [
		combat_state.turn_index,
		combat_state.player_actor.hp,
		combat_state.player_actor.max_hp,
		combat_state.player_actor.block,
		ui.player_status_text(),
		combat_state.enemy_actor.hp,
		combat_state.enemy_actor.max_hp,
...
func start_encounter() -> void:
	...
	context._begin_phase(BattlePhases.PRE_BATTLE, {"encounter": encounter_index})
	...
	reward_flow.set_discard_overlay_visible(false)
	...
	context._end_phase(BattlePhases.PRE_BATTLE, {"encounter": encounter_index})
	...
	context._begin_phase(BattlePhases.BATTLE_START, {"enemy": enemy_name, "difficulty": combat_state.combat_difficulty})
	...
	context._end_phase(BattlePhases.BATTLE_START, {"enemy": enemy_name, "difficulty": combat_state.combat_difficulty})
	context._begin_phase(BattlePhases.DECK_PREP, {"enemy_deck": combat_state.enemy_actor.draw_pile.size()})
	draw_enemy_cards(context.ENEMY_HAND_SIZE)
	refresh_enemy_intent()
	context._end_phase(BattlePhases.DECK_PREP, {"enemy_hand": combat_state.enemy_actor.hand.size()})
	combat_state.next_step = "encounter"
	ui.setup_portraits(combat_state.enemy_data)
	RunState.log_event("遭遇魔物：%s" % combat_state.enemy_data.get("name", "未知魔物"))
	ui.reset_battle_log()
	combat_state.turn_index = 1
	ui.append_battle_log("遭遇魔物：%s（HP %d/%d）" % [
	...
	if RunState.next_encounter_first_strike:
		...
		ui.append_battle_log("先手出击：造成%d点伤害（敌人HP %d/%d）。" % [
			strike_damage,
			combat_state.enemy_actor.hp,
			combat_state.enemy_actor.max_hp
		])
		...
		ui.append_battle_log("你稳住气息，准备战斗。")
	context._begin_phase(BattlePhases.DRAW_CARDS, {"count": context.HAND_DRAW_FIRST})
	draw_cards(context.HAND_DRAW_FIRST)
	context._end_phase(BattlePhases.DRAW_CARDS, {"hand": combat_state.player_actor.hand.size()})
	context._begin_phase(BattlePhases.STATUS_RESOLVE, {"stage": "player_start", "turn": combat_state.turn_index})
	context._end_phase(BattlePhases.STATUS_RESOLVE, {"stage": "player_start", "turn": combat_state.turn_index})
	ui.update_ui()
	RunState.save_run()
...
func on_hand_card_clicked(card_id: String, index: int) -> void:
	...
	if bool(card_data.get("unplayable", false)):
		ui.append_battle_log("【%s】无法打出。" % card_name)
		return
	...
	if cost > combat_state.player_actor.energy:
		ui.append_battle_log("能量不足，无法打出【%s】。" % card_name)
		return
	context._begin_phase(BattlePhases.USE_CARD, {"card": card_name, "cost": cost})
	combat_state.player_actor.energy -= cost
	combat_state.player_next_card_cost_delta = 0
	context._begin_phase(BattlePhases.CARD_EFFECT, {"card": card_name, "card_data": card_data, "source": "player"})
	context._end_phase(BattlePhases.CARD_EFFECT, {"card": card_name})
	var removed = remove_card_from_hand(index)
	if removed != null:
		if bool(card_data.get("exhaust", false)):
			ui.append_battle_log("【%s】已消耗。" % card_name)
		else:
			combat_state.player_actor.discard_pile.append(removed)
	context._begin_phase(BattlePhases.TARGET_REACTION, {"card": card_name, "source": "player"})
	context._end_phase(BattlePhases.TARGET_REACTION, {"card": card_name})
	context._begin_phase(BattlePhases.FINAL_RESOLVE, {"card": card_name})
	ui.update_ui()
	RunState.save_run()
	context._end_phase(BattlePhases.FINAL_RESOLVE, {"card": card_name})
	context._end_phase(BattlePhases.USE_CARD, {"card": card_name})
	context._begin_phase(BattlePhases.NEXT_CARD, {"hand": combat_state.player_actor.hand.size()})
	context._end_phase(BattlePhases.NEXT_CARD, {"hand": combat_state.player_actor.hand.size()})
...
func apply_player_damage(amount: int, pierce: bool) -> int:
	...
	if actual > 0:
		...
		ui.play_enemy_hit_effect()
		if combat_state.equip_block_on_damage > 0:
			if gain_player_block(combat_state.equip_block_on_damage):
				ui.append_battle_log("血纹护符触发：护甲+%d（护甲 %d）。" % [combat_state.equip_block_on_damage, combat_state.player_actor.block])
				context.sfx_block.play()
		if combat_state.power_bleed_on_damage > 0:
			combat_state.enemy_bleed += combat_state.power_bleed_on_damage
			ui.append_battle_log("血炼触发：流血+%d（%d）。" % [combat_state.power_bleed_on_damage, combat_state.enemy_bleed])
	return actual
...
func apply_ethereal_cleanup() -> void:
	for index in range(combat_state.player_actor.hand.size() - 1, -1, -1):
		...
		if bool(card_data.get("ethereal", false)):
			combat_state.player_actor.hand.remove_at(index)
			ui.append_battle_log("【%s】虚无消散。" % card_data.get("name", "卡牌"))
...
func on_end_turn_pressed() -> void:
	...
	if not combat_state.end_turn_phase_pending:
		...
		context._begin_phase(BattlePhases.END_TURN_TRIGGER, {"turn": combat_state.turn_index})
	apply_ethereal_cleanup()
	if combat_state.player_actor.hand.size() > context.HAND_LIMIT:
		reward_flow.open_discard_overlay(combat_state.player_actor.hand.size() - context.HAND_LIMIT)
		return
	context._end_phase(BattlePhases.END_TURN_TRIGGER, {"turn": combat_state.turn_index, "discard_required": 0})
...
func resolve_end_turn() -> void:
	...
	context._begin_phase(BattlePhases.END_TURN_RESOLVE, {"turn": resolved_turn})
	context._begin_phase(BattlePhases.STATUS_RESOLVE, {"stage": "player_end", "turn": resolved_turn})
	context._end_phase(BattlePhases.STATUS_RESOLVE, {"stage": "player_end", "turn": resolved_turn})
	await enemy_turn()
	if combat_state.combat_over:
		context._end_phase(BattlePhases.END_TURN_RESOLVE, {"turn": resolved_turn, "combat_over": true})
		combat_state.turn_locked = false
		ui.update_ui()
		return
	log_turn_end()
	...
	log_turn_start()
	combat_state.turn_locked = false
	context._end_phase(BattlePhases.END_TURN_RESOLVE, {"turn": resolved_turn})
	ui.update_ui()
	RunState.save_run()

func enemy_turn() -> void:
	var enemy_turn := combat_state.turn_index
	context._begin_phase(BattlePhases.ENEMY_TURN_START, {"turn": enemy_turn})
	combat_state.enemy_acting = true
	combat_state.enemy_actor.energy = RunState.energy_max
	ui.update_ui()
	if combat_state.player_skip_enemy_turn:
		combat_state.player_skip_enemy_turn = false
		ui.append_battle_log("停滞结界生效：魔物本回合无法行动。")
	...
	if combat_state.player_actor.hp <= 0:
		...
		ui.append_battle_log("你在山道上倒下，征途告终。")
		RunState.log_event("你在山道上倒下。")
		RunState.finalize_run_score()
		RunState.reset_after_run()
		context._end_phase(BattlePhases.ENEMY_TURN_START, {"turn": enemy_turn, "combat_over": true})
		return
	if not combat_state.combat_over:
		context._begin_phase(BattlePhases.STATUS_RESOLVE, {"stage": "enemy_end", "turn": enemy_turn})
		context._end_phase(BattlePhases.STATUS_RESOLVE, {"stage": "enemy_end", "turn": enemy_turn})
		if combat_state.enemy_actor.hp <= 0:
			check_enemy_defeat()
			context._end_phase(BattlePhases.ENEMY_TURN_START, {"turn": enemy_turn, "combat_over": combat_state.combat_over})
			return
	if not combat_state.combat_over:
...
func check_enemy_defeat() -> void:
	if combat_state.enemy_actor.hp <= 0:
		context._begin_phase(BattlePhases.BATTLE_END_TRIGGER, {"turn": combat_state.turn_index})
		combat_state.combat_over = true
		log_turn_end()
		ui.append_battle_log("敌人倒下，战斗结束。")
		context._end_phase(BattlePhases.BATTLE_END_TRIGGER, {"turn": combat_state.turn_index})
		context._begin_phase(BattlePhases.BATTLE_END_RESOLVE, {"turn": combat_state.turn_index})
		...
		if combat_state.run_complete:
			ui.append_battle_log("你征服了 %s，登顶通关！" % GameData.MOUNTAIN_NAME)
			RunState.log_event("登顶通关，征服 %s。" % GameData.MOUNTAIN_NAME)
			RunState.finalize_run_score()
			RunState.reset_after_run()
			context._end_phase(BattlePhases.BATTLE_END_RESOLVE, {"turn": combat_state.turn_index, "result": "complete"})
			context._begin_phase(BattlePhases.BATTLE_END, {"result": "complete"})
			context._end_phase(BattlePhases.BATTLE_END, {"result": "complete"})
		else:
			RunState.log_event("击退了 %s。" % combat_state.enemy_data.get("name", "魔物"))
			context._end_phase(BattlePhases.BATTLE_END_RESOLVE, {"turn": combat_state.turn_index, "result": "continue"})
...
func apply_enemy_card(card_data: Dictionary) -> void:
	...
	if enemy_card_targets_player(intent_type) and combat_state.player_nullify_count > 0:
		combat_state.player_nullify_count -= 1
		ui.append_battle_log("护幕抵消了魔物的【%s】。" % card_name)
		return
	var damage := int(round(int(card_data.get("damage", 0)) * combat_state.enemy_power_mult))
	var block := int(round(int(card_data.get("block", 0)) * combat_state.enemy_power_mult))
	var heal := int(round(int(card_data.get("heal", 0)) * combat_state.enemy_power_mult))
	...
	match intent_type:
		"attack":
			...
			if dealt > 0:
				ui.append_battle_log("魔物使用【%s】->你：造成%d点伤害（你HP %d/%d，护甲 %d）。" % [
					card_name,
					dealt,
					combat_state.player_actor.hp,
					combat_state.player_actor.max_hp,
				...
				ui.append_battle_log("魔物使用【%s】->你：攻击被护甲挡住（你护甲 %d）。" % [card_name, combat_state.player_actor.block])
		"multi_attack":
			...
			if dealt > 0:
				ui.append_battle_log("魔物使用【%s】->你：连击%d次造成%d点伤害（你HP %d/%d，护甲 %d）。" % [
					card_name,
					hits,
					dealt,
					combat_state.player_actor.hp,
				...
				ui.append_battle_log("魔物使用【%s】->你：连击被护甲挡住（你护甲 %d）。" % [card_name, combat_state.player_actor.block])
		"guard":
			...
			if combat_state.enemy_block_gain_reduction > 0:
				ui.append_battle_log("魔物使用【%s】->自己：护甲+%d（被削减 %d，护甲 %d）。" % [
					card_name,
					gain,
				...
				ui.append_battle_log("魔物使用【%s】->自己：护甲+%d（护甲 %d）。" % [card_name, gain, combat_state.enemy_actor.block])
		"charge":
			combat_state.enemy_attack_bonus += charge
			ui.append_battle_log("魔物使用【%s】->自己：蓄力+%d（下次攻击加成 %d）。" % [
				card_name,
				charge,
		...
		"drain":
			...
			if dealt > 0 and heal > 0:
				...
				ui.append_battle_log("魔物使用【%s】->你：造成%d点伤害（你HP %d/%d）。自身恢复%d点生命（敌人HP %d/%d）。" % [
					card_name,
					dealt,
					combat_state.player_actor.hp,
					combat_state.player_actor.max_hp,
				...
				ui.append_battle_log("魔物使用【%s】->你：造成%d点伤害（你HP %d/%d）。" % [
					card_name,
					dealt,
					combat_state.player_actor.hp,
					combat_state.player_actor.max_hp
		...
		"heal":
			if heal > 0:
				combat_state.enemy_actor.hp = min(combat_state.enemy_actor.hp + heal, combat_state.enemy_actor.max_hp)
				ui.append_battle_log("魔物使用【%s】->自己：恢复%d点生命（敌人HP %d/%d）。" % [
					card_name,
					heal,
					combat_state.enemy_actor.hp,
					combat_state.enemy_actor.max_hp
		...
		"debuff":
			...
			ui.append_battle_log("魔物使用【%s】->你：施加%s（你状态：%s）。" % [
				card_name,
				enemy_debuff_summary(card_data),
				ui.player_status_text()
			])
		"attack_debuff":
			...
			if dealt > 0:
				ui.append_battle_log("魔物使用【%s】->你：造成%d点伤害（你HP %d/%d），施加%s（你状态：%s）。" % [
					card_name,
					dealt,
					combat_state.player_actor.hp,
					combat_state.player_actor.max_hp,
				...
				ui.append_battle_log("魔物使用【%s】->你：攻击被护甲挡住，施加%s（你状态：%s）。" % [
					card_name,
					status_summary,
					ui.player_status_text()
				])
		_:
			ui.append_battle_log("魔物踌躇不前。")
	...
	if damage > 0:
		...
		if combat_state.power_first_damage_block > 0 and not combat_state.power_first_damage_block_used:
			...
			if gain_player_block(combat_state.power_first_damage_block):
				ui.append_battle_log("坚毅之魂触发：护甲+%d（护甲 %d）。" % [combat_state.power_first_damage_block, combat_state.player_actor.block])
				context.sfx_block.play()
		if combat_state.player_counter_ratio > 0.0:
			...
			if counter_damage > 0:
				...
				if dealt > 0:
					ui.append_battle_log("反击造成%d点伤害（敌人HP %d/%d）。" % [
						dealt,
						combat_state.enemy_actor.hp,
						combat_state.enemy_actor.max_hp
					])
		if combat_state.player_damage_draw > 0:
			draw_cards(combat_state.player_damage_draw)
			ui.append_battle_log("补给回响：抽%d张（手牌 %d）。" % [combat_state.player_damage_draw, combat_state.player_actor.hand.size()])
	return damage
