...
func _apply_card_effect(card_data: Dictionary) -> void:
	...
	var attack_chain_before := context.player_attack_chain
	if is_attack:
		context.player_attack_chain += 1
		context.player_defend_chain = 0
	elif is_defend:
		context.player_defend_chain += 1
	...
	if damage > 0:
		...
		if dealt > 0:
			...
			context._append_battle_log("你使用【%s】->敌人：%s造成%d点伤害（敌人HP %d/%d，护甲 %d）。" % [
				...
				context.enemy_actor.block
			])
		else:
			context._append_battle_log("你使用【%s】->敌人：攻击被护甲挡住（敌人护甲 %d）。" % [card_name, context.enemy_actor.block])
		if dealt > 0:
			...
			if lifesteal_ratio > 0.0:
				...
				heal_amount = min(heal_amount, context.player_actor.max_hp - context.player_actor.hp)
				if heal_amount > 0:
					context.player_actor.hp += heal_amount
					context._sync_player_hp()
					context._append_battle_log("【%s】吸血恢复%d点生命（HP %d/%d）。" % [
						card_name,
						heal_amount,
						context.player_actor.hp,
						context.player_actor.max_hp
					])
	if is_attack and context.player_bleed_on_attack > 0:
		context.enemy_bleed += context.player_bleed_on_attack
		context._append_battle_log("血痕持续扩散，敌人流血+%d（%d）。" % [context.player_bleed_on_attack, context.enemy_bleed])
	if is_attack and context.player_attack_bonus_on_attack > 0:
		context.player_damage_bonus_turn += context.player_attack_bonus_on_attack
		context._append_battle_log("嗜战叠加，攻击伤害+%d（本回合 +%d）。" % [context.player_attack_bonus_on_attack, context.player_damage_bonus_turn])
	if is_attack and context.power_first_attack_draw > 0 and not context.power_first_attack_draw_used:
		context.power_first_attack_draw_used = true
		context._draw_cards(context.power_first_attack_draw)
		context._append_battle_log("迅捷心法触发：抽%d张（手牌 %d）。" % [context.power_first_attack_draw, context.player_actor.hand.size()])
	if is_attack and context.equip_attack_chain_draw > 0 and context.player_attack_chain >= 2:
		context._draw_cards(context.equip_attack_chain_draw)
		context._append_battle_log("连击腕轮触发：抽%d张（手牌 %d）。" % [context.equip_attack_chain_draw, context.player_actor.hand.size()])
		context.player_attack_chain = 0
	if is_defend and context.equip_defend_chain_block > 0 and context.player_defend_chain >= 2:
		context._gain_player_block(context.equip_defend_chain_block)
		context._append_battle_log("守势腰带触发：护甲+%d（护甲 %d）。" % [context.equip_defend_chain_block, context.player_actor.block])
		context.sfx_block.play()
		context.player_defend_chain = 0
	var block := int(card_data.get("block", 0))
	if block > 0:
		if context._gain_player_block(block):
			context._append_battle_log("你使用【%s】->自己：护甲+%d（护甲 %d）。" % [card_name, block, context.player_actor.block])
			context.sfx_block.play()
		else:
	...
	if draw_count > 0:
		...
		context._append_battle_log("你使用【%s】->自己：抽%d张（手牌 %d，抽牌堆 %d）。" % [
			card_name,
			draw_count,
			context.player_actor.hand.size(),
			context.player_actor.draw_pile.size()
		])
	var heal := int(card_data.get("heal", 0))
	if heal > 0:
		var before: int = context.player_actor.hp
		context.player_actor.hp = min(context.player_actor.hp + heal, context.player_actor.max_hp)
		var healed: int = context.player_actor.hp - before
		context._sync_player_hp()
	...
	if apply_bleed > 0:
		context.enemy_bleed += apply_bleed
		context._append_battle_log("你使用【%s】->敌人：流血+%d（%d）。" % [card_name, apply_bleed, context.enemy_bleed])
	var apply_poison := int(card_data.get("apply_poison", 0))
	if apply_poison > 0:
		context.enemy_poison += apply_poison
		context._append_battle_log("你使用【%s】->敌人：中毒+%d（%d）。" % [card_name, apply_poison, context.enemy_poison])
	var apply_burn := int(card_data.get("apply_burn", 0))
	if apply_burn > 0:
		context.enemy_burn += apply_burn
		context._append_battle_log("你使用【%s】->敌人：灼烧+%d（%d）。" % [card_name, apply_burn, context.enemy_burn])
	var charge_mult := float(card_data.get("charge_mult", 0.0))
	if charge_mult > 0.0:
		context.player_next_attack_mult = max(context.player_next_attack_mult, charge_mult)
		context._append_battle_log("你使用【%s】->自己：蓄力x%.1f。" % [card_name, context.player_next_attack_mult])
	if bool(card_data.get("skip_enemy_turn", false)):
		context.player_skip_enemy_turn = true
	...
	if counter_ratio > 0.0:
		context.player_counter_ratio = max(context.player_counter_ratio, counter_ratio)
		context._append_battle_log("你使用【%s】->自己：反击比例提升至%.0f%%。" % [card_name, context.player_counter_ratio * 100.0])
	var nullify_count := int(card_data.get("nullify_count", 0))
	if nullify_count > 0:
		context.player_nullify_count += nullify_count
		context._append_battle_log("你使用【%s】->自己：护幕生效（%d次）。" % [card_name, context.player_nullify_count])
	var damage_draw := int(card_data.get("damage_draw", 0))
	if damage_draw > 0:
		context.player_damage_draw += damage_draw
		context._append_battle_log("你使用【%s】->自己：受伤抽牌+%d（本回合 %d）。" % [card_name, damage_draw, context.player_damage_draw])
	var bleed_on_attack := int(card_data.get("bleed_on_attack", 0))
	if bleed_on_attack > 0:
		context.player_bleed_on_attack += bleed_on_attack
		context._append_battle_log("你使用【%s】->自己：每次攻击流血+%d（本回合 %d）。" % [card_name, bleed_on_attack, context.player_bleed_on_attack])
	var attack_bonus_on_attack := int(card_data.get("attack_bonus_on_attack", 0))
	if attack_bonus_on_attack > 0:
		context.player_attack_bonus_on_attack += attack_bonus_on_attack
		context._append_battle_log("你使用【%s】->自己：每次攻击伤害+%d（本回合 %d）。" % [card_name, attack_bonus_on_attack, context.player_attack_bonus_on_attack])
	var next_bonus := int(card_data.get("next_attack_bonus", 0))
	if next_bonus > 0:
		context.player_next_attack_bonus += next_bonus
	if bool(card_data.get("next_attack_pierce", false)):
		context.player_next_attack_pierce = true
	if next_bonus > 0 or context.player_next_attack_pierce:
	...
	if enemy_block_reduce > 0:
		context.enemy_block_gain_reduction = max(context.enemy_block_gain_reduction, enemy_block_reduce)
		context._append_battle_log("你使用【%s】->敌人：本回合护甲获得-%d。" % [card_name, context.enemy_block_gain_reduction])
	var cost_delta := int(card_data.get("next_card_cost_delta", 0))
	if cost_delta != 0:
		context.player_next_card_cost_delta += cost_delta
		context._append_battle_log("你使用【%s】->自己：下一张牌费用%+d。" % [card_name, cost_delta])
	if bool(card_data.get("block_disabled", false)):
		context.player_block_disabled = true
		context._append_battle_log("你使用【%s】->自己：本回合无法获得护甲。" % card_name)
	var equip_attack := int(card_data.get("equip_attack_bonus", 0))
	if equip_attack > 0:
		context.equip_attack_bonus += equip_attack
		context._append_battle_log("你装备【%s】：攻击伤害+%d（本场战斗 %d）。" % [card_name, equip_attack, context.equip_attack_bonus])
	var equip_reduce := int(card_data.get("equip_damage_reduction", 0))
	if equip_reduce > 0:
		context.equip_damage_reduction += equip_reduce
		context._append_battle_log("你装备【%s】：受到伤害-%d（本场战斗 %d）。" % [card_name, equip_reduce, context.equip_damage_reduction])
	var equip_attack_draw := int(card_data.get("equip_attack_chain_draw", 0))
	if equip_attack_draw > 0:
		context.equip_attack_chain_draw += equip_attack_draw
	...
	if equip_defend_block > 0:
		context.equip_defend_chain_block += equip_defend_block
	...
	if equip_block_on_hit > 0:
		context.equip_block_on_damage += equip_block_on_hit
	...
	if equip_bleed_bonus > 0:
		context.equip_bleed_bonus_per_stack += equip_bleed_bonus
	...
	if power_attack_draw > 0:
		context.power_first_attack_draw += power_attack_draw
	...
	if power_damage_block > 0:
		context.power_first_damage_block += power_damage_block
	...
	if power_bleed > 0:
		context.power_bleed_on_damage += power_bleed
	...
	if is_attack:
		context.player_next_attack_mult = 1.0
		context.player_next_attack_bonus = 0
		context.player_next_attack_pierce = false
...
func _calculate_attack_damage(card_data: Dictionary, combo_count: int) -> int:
	...
	if bool(card_data.get("damage_from_block", false)):
		bonus += context.player_actor.block
	if bool(card_data.get("damage_from_missing_hp", false)):
		var missing := context.player_actor.max_hp - context.player_actor.hp
	...
	if hp_ratio > 0.0:
		bonus += int(round(float(context.player_actor.hp) * hp_ratio))
	var per_combo := int(card_data.get("damage_per_attack_chain", 0))
	if per_combo > 0:
		bonus += per_combo * combo_count
	var total := base_damage + bonus + context.equip_attack_bonus + context.player_damage_bonus_turn + context.player_next_attack_bonus
	var low_hp_mult := float(card_data.get("low_hp_mult", 1.0))
	if low_hp_mult > 1.0:
		var threshold := float(card_data.get("low_hp_threshold", 0.5))
		if float(context.player_actor.hp) <= float(context.player_actor.max_hp) * threshold:
	...
	if execute_threshold > 0.0:
		if context.enemy_actor.max_hp > 0 and float(context.enemy_actor.hp) <= float(context.enemy_actor.max_hp) * execute_threshold:
			var execute_mult := float(card_data.get("execute_mult", 2.0))
			total = int(round(float(total) * execute_mult))
	if context.player_next_attack_mult > 1.0:
		total = int(round(float(total) * context.player_next_attack_mult))
...
func _should_pierce(card_data: Dictionary) -> bool:
	...
	if pierce_threshold > 0 and context.player_actor.block >= pierce_threshold:
		return true
	if context.player_next_attack_pierce:
		return true
	return false
