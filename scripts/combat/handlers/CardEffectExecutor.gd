...
func _apply_card_effect(card_data: Dictionary) -> void:
	...
	if damage > 0:
		var dealt: int = context.combat_flow.apply_player_damage(damage, pierce)
		if dealt > 0:
			var prefix := "穿刺" if pierce else ""
			context.battle_log_panel.append_line("你使用【%s】->敌人：%s造成%d点伤害（敌人HP %d/%d，护甲 %d）。" % [
				card_name,
				prefix,
				dealt,
			...
			context.battle_log_panel.append_line("你使用【%s】->敌人：攻击被护甲挡住（敌人护甲 %d）。" % [card_name, context.combat_state.enemy_actor.block])
		if dealt > 0:
			...
			if lifesteal_ratio > 0.0:
				...
				if heal_amount > 0:
					...
					context.combat_flow.sync_player_hp()
					context.battle_log_panel.append_line("【%s】吸血恢复%d点生命（HP %d/%d）。" % [
						card_name,
						heal_amount,
	...
	if block > 0:
		if context.combat_flow.gain_player_block(block):
			context.battle_log_panel.append_line("你使用【%s】->自己：护甲+%d（护甲 %d）。" % [card_name, block, context.combat_state.player_actor.block])
			context.sfx_block.play()
		else:
			context.battle_log_panel.append_line("你使用【%s】->自己：护甲被封锁，无法获得护甲。" % card_name)
	var draw_count := int(card_data.get("draw", 0))
	if draw_count > 0:
		context.combat_flow.draw_cards(draw_count)
		context.battle_log_panel.append_line("你使用【%s】->自己：抽%d张（手牌 %d，抽牌堆 %d）。" % [
			card_name,
			draw_count,
	...
	if heal > 0:
		...
		context.combat_flow.sync_player_hp()
		context.battle_log_panel.append_line("你使用【%s】->自己：恢复%d点生命（HP %d/%d）。" % [
			card_name,
			healed,
			context.combat_state.player_actor.hp,
			context.combat_state.player_actor.max_hp
	...
	if apply_bleed > 0:
		...
		context.battle_log_panel.append_line("你使用【%s】->敌人：流血+%d（%d）。" % [card_name, apply_bleed, context.combat_state.enemy_bleed])
	var apply_poison := int(card_data.get("apply_poison", 0))
	if apply_poison > 0:
		context.combat_state.enemy_poison += apply_poison
		context.battle_log_panel.append_line("你使用【%s】->敌人：中毒+%d（%d）。" % [card_name, apply_poison, context.combat_state.enemy_poison])
	...
	if equip_attack > 0:
		...
		context.battle_log_panel.append_line("你装备【%s】：攻击伤害+%d（本场战斗 %d）。" % [card_name, equip_attack, context.combat_state.equip_attack_bonus])
	var equip_reduce := int(card_data.get("equip_damage_reduction", 0))
	if equip_reduce > 0:
		context.combat_state.equip_damage_reduction += equip_reduce
		context.battle_log_panel.append_line("你装备【%s】：受到伤害-%d（本场战斗 %d）。" % [card_name, equip_reduce, context.combat_state.equip_damage_reduction])
	...
	if equip_attack_draw > 0:
		...
		context.battle_log_panel.append_line("你装备【%s】：连击抽牌+%d。" % [card_name, equip_attack_draw])
	...
	if equip_defend_block > 0:
		...
		context.battle_log_panel.append_line("你装备【%s】：守势护甲+%d。" % [card_name, equip_defend_block])
	...
	if equip_block_on_hit > 0:
		...
		context.battle_log_panel.append_line("你装备【%s】：造成伤害时护甲+%d。" % [card_name, equip_block_on_hit])
	...
	if equip_bleed_bonus > 0:
		...
		context.battle_log_panel.append_line("你装备【%s】：流血伤害提升。" % card_name)
	...
	if power_attack_draw > 0:
		...
		context.battle_log_panel.append_line("你领悟【%s】：每回合首攻抽%d张。" % [card_name, power_attack_draw])
	...
	if power_damage_block > 0:
		...
		context.battle_log_panel.append_line("你领悟【%s】：每回合首次受伤护甲+%d。" % [card_name, power_damage_block])
	var power_bleed := int(card_data.get("power_bleed_on_damage", 0))
	if power_bleed > 0:
		context.combat_state.power_bleed_on_damage += power_bleed
		context.battle_log_panel.append_line("你领悟【%s】：伤害附加流血+%d。" % [card_name, power_bleed])
