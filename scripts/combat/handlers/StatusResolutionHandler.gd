...
func _resolve_player_end() -> void:
	if context.combat_state.player_weak_turns > 0:
		context.combat_state.player_weak_turns -= 1
	context.combat_state.player_bleed_on_attack = 0
	context.combat_state.player_attack_bonus_on_attack = 0
	context.combat_state.player_damage_bonus_turn = 0
...
func _resolve_enemy_end() -> void:
	...
	if context.combat_state.player_vulnerable_turns > 0:
		context.combat_state.player_vulnerable_turns -= 1
	context.combat_state.player_counter_ratio = 0.0
	context.combat_state.player_nullify_count = 0
	context.combat_state.player_damage_draw = 0
	context.combat_state.player_block_disabled = false
	...
	context.combat_state.player_skip_enemy_turn = false
